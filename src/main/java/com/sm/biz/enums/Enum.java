...
import com.sm.biz.enums.internal.EnumConstant;
import com.sm.biz.enums.internal.NumberType;
import com.sm.common.libs.util.ClassLoaderUtil;

/**
 * 类型安全的枚举类型.
 * 
 * @author <a href="chenxu.xc@alibaba-inc.com">xc</a>
 * @version create on 2017年11月10日 下午11:35:12
 */
public abstract class Enum extends Number implements NumberType, Comparable<Object>, Serializable {
  ...
  private static <E extends Enum> E createEnum(String name, Number value, boolean withValue) {
    ...
    EnumType enumType = EnumUtil.getEnumType(enumObject.getClass());
    boolean flagMode = Flags.class.isInstance(enumObject);
  ...
  public String getName() {
    if (name == null) {
      Class<? extends Enum> enumClass = ensureClassLoaded();
   ...
   */
  protected Enum setName(String name) {
    if (this.name != null) {
  ...
  public Class<? extends Enum> ensureClassLoaded() {
    Class<? extends Enum> enumClass = getClass();
  ...
  protected Object readResolve() throws ObjectStreamException {
    ...
    if (!enumObject.value.equals(value)) {
      throw new InvalidClassException("Enum value \"" + value + "\" does not match in class " + enumClass.getName());
    }
        ...
        for (int i = 0; i < fields.length; i++) {
          ...
          if (Modifier.isPublic(modifier) && Modifier.isFinal(modifier) && Modifier.isStatic(modifier)) {
            try {
              ...
              for (Iterator<Enum> j = valueMap.values().iterator(); j.hasNext();) {
                Enum enumObject = j.next();
