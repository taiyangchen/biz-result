package com.sm.biz.resourcebundle.xml;
...
import com.sm.biz.resourcebundle.AbstractResourceBundleFactory;
...
import com.sm.biz.resourcebundle.ResourceBundleLoader;
import com.sm.biz.resourcebundle.proxy.AutoReloadResourceBundleProxy;
import com.sm.biz.resourcebundle.proxy.ConvertResourceBundleProxy;
...
public class XMLResourceBundleFactory extends AbstractResourceBundleFactory {
  ...
  protected AbstractResourceBundle parse(InputStream stream, String systemId) throws ResourceBundleCreateException {
    try {
      ...
      if (StringUtils.isNotEmpty(reference)) {// 有引用项
        ...
        if (StringUtils.isNotEmpty(charConverter)) {// 使用自动字符转换
          if (logger.isDebugEnabled()) {
      ...
      if (ResourceBundleFactory.isAutoReload()) {
        ...
        resourceBundle = new AutoReloadResourceBundleProxy(resourceBundle, this, bundleName,
