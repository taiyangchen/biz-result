...
import com.sm.biz.resourcebundle.xml.XMLResourceBundleFactory;
import com.sm.common.libs.able.CloneableObject;
import com.sm.common.libs.collection.SoftHashMap;
 ...
 * @version create on 2017年11月10日 下午11:49:12
 */
public abstract class ResourceBundleFactory implements ResourceBundleConstant {
   ...
   * 如果设置为true，则每次获取ResourceBundle时检测相应的配置文件最后修改时间是否大于cache中缓存的修改时间， 如果大于则清除相关缓存，重新加载相关配置并保存在cache中。
  ...
  private static final class Helper {
     ...
     * @throws java.util.MissingResourceException 指定bundle未找到, 或创建bundle错误
     */
    private static AbstractResourceBundle getBundleImpl(String baseName, Locale locale, ResourceBundleFactory factory) {
      if (baseName == null) {
      ...
      try {
        ...
        List<Object> bundlesFound = new ArrayList<>(MAX_BUNDLES_SEARCHED);
    ...
    private static List<String> calculateBundleNames(String baseName, Locale locale) {
      final List<String> result = new ArrayList<>(MAX_BUNDLES_SEARCHED);
    ...
    private static void throwResourceBundleException(boolean missing, String baseName, Locale locale, Throwable cause) {
      ...
      if (missing) {
        throw new ResourceBundleException(ResourceBundleConstant.RB_MISSING_RESOURCE_BUNDLE,
            new Object[] {baseName, locale}, bundleName, "", cause);
      }
  ...
  private static final class CacheKey implements CloneableObject<CacheKey> {
    ...
    public String toString() {
      return new StringBuilder("CacheKey[factory=").append((factoryRef == null) ? "null" : factoryRef.get())
          .append(", bundleName=").append(bundleName).append(", defaultLocale=").append(defaultLocale).append("]")
          .toString();
    }
  }
